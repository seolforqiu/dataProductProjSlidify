...
```{r,echo=FALSE, eval=FALSE, fig.cap="Figure 1. Contingency Analysis of Conversion By Landing Page", fig.aligh="center"}
ctrlClick <- 353
trtClick <- 401
ctrlTotal <- 9985
trtTotal <- 10008
contTable <- data.frame(row.names = c("Control", "Treatment"), 
                        Converted = c(ctrlClick,trtClick), 
                        Nonconverted= c(ctrlTotal - ctrlClick, trtTotal - trtClick))
...
test <- chisq.test(contTable,simulate.p.value = FALSE)
pvalue <- test$p.value
test
...
### Conclusion
Treatment group is `r if(pvalue >= 0.05) "not "`significantly ( $\alpha$    = 0.05 ) different from control group.
